...
import com.example.appweatherforecast.databinding.ActivityHourlyWeatherBinding;
...
public class HourlyForecastActivity extends MainActivity {

    private ActivityHourlyWeatherBinding binding;
    private RecyclerView recyclerViewHourlyWeather;
    private HourlyWeatherAdapter hourlyWeatherAdapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityHourlyWeatherBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());


        recyclerViewHourlyWeather = findViewById(R.id.recyclerViewHourlyWeather);
        recyclerViewHourlyWeather.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false));
        ...
        fetchHourlyWeatherData(cityName); // Thay thế "London" bằng tên thành phố bạn muốn truy vấn.
        ...
        binding.next7day.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String cityname =binding.cityname.getText().toString();
                String temp =binding.temp.getText().toString();
                Intent intent = new Intent(HourlyForecastActivity.this, DailyForecastActivity.class);
    ...
    private void fetchHourlyWeatherData(String cityname) {
            ...
            public void onResponse(@NonNull Call<HourlyWeatherResponse> call, @NonNull Response<HourlyWeatherResponse> response) {
                if (response.isSuccessful()) {
                    ...
                    if (hourlyWeatherResponse != null && hourlyWeatherResponse.getHourlyWeatherItemList() != null) {
                        ...
                        hourlyWeatherAdapter = new HourlyWeatherAdapter(hourlyWeatherItemList);
                        recyclerViewHourlyWeather.setAdapter(hourlyWeatherAdapter);
                    } else {
                        Toast.makeText(HourlyForecastActivity.this, "Không tìm thấy thông tin thời tiết hàng giờ.", Toast.LENGTH_SHORT).show();
